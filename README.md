...
* 支持 复制图片、保存单张图片，也可使用 顶部菜单「导出」-「导出全部图片资源」菜单保存全部图片文件，保存位置为`export/img/`目录下。
...
* 可保存单张地图图片，也可使用 顶部菜单「导出」-「导出全部地图图片」菜单保存全部地图图片文件，保存位置为`export/map/`目录下。
* 地图图片默认命名格式`{区域短编号}-{地名}-{二级地名}.png`。

### 4. 音乐列表

* 左侧列表展示全部音乐列表，支持筛选、仅显示新增项功能。
