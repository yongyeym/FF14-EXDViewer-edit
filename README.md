...
Release发布文件选择：
* `ff14-exdviewer-edit.exe`：不包含其他任何独立额外工具的单个可执行文件，运行产生的文件会放到exe同目录下，建议放到单独文件夹内再使用。
* `column_layout.json`：由我维护的数据表结构定义中文化配置，映射数据基于EXDSchema，需放到exe目录的`config/`文件夹内，下载单exe版本时推荐一起下载此文件。此文件用于启用数据表页面的个性化配置表格展示功能，详见下方次要功能说明#3。
* `ff14-exdviewer-edit.zip`：完整包，包含`column_layout.json`和多个独立额外工具exe文件，独立额外工具不影响程序主体功能，仅导出当前数据表内容Diff总结表功能需使用，详见下方数据功能说明。
...
* 左侧列表各数据表名称前方有空心星星图标，点击后变为实心黄色⭐，代表将此数据表收藏，被收藏的表会在列表中置顶。
...
* 音乐默认命名格式为`游戏音乐原始文件名.{ogg,wav,hca…}`。
...
* 数据表、地图、音乐页面左侧列表上方，图片列表的右上方工具栏区域，均可使用「仅显示新增项」开关按钮（🔍），鼠标悬停在按钮上会提示有多少个新增项。
...
* 当前版本与对比有变动版本的相关数据列表存档文件位于`config/{img_list,map_list,music_list,sheet_list}_{游戏版本号}.json`，始终至多保留两个版本。
* 不再使用的旧版本数据列表会归档到 `config/bak/{img_list,map_list,music_list,sheet_list}_{游戏版本号}.json`，可自行删除。
...
1. 点击数据表表格区域右上角工具栏「版本Diff」按钮，打开对比窗口。
...
* 仅筛选关键列变更：默认不启用。只对比EXDSchema的yml文件中定义的数据表主展示列的数据是否有变更，此列名后会有黄色星星⭐标记，yml文件中由配置项`displayField:`定义，此功能主要用于物品/装备/副本等数据表大规模修改了可用职业范围的情况（新增职业）。
...
> 个性化配置表格仍需要原始EXDSchema结构定义表作为基础，且json配置直接对应EXDSchema中列名。
> 
> EXDSchema本身为社区玩家合作维护的结构定义表，会不断更新，若EXDSchema后续更新了我的json配置中映射的列名名称，相关项会因找不到原始列名而被隐藏。
> 
> 目前json配置使用了EXDSchema中部分命名为未知（Unknown）的列，这些列名更可能因更新而失效，请等待作者更新，也欢迎提出issue或commit合作维护。
...
|**找出版本变更的数据表**|选择两个版本文件夹，对比其中的CSV文件，找出有差异的文件并将结果存为txt文档，并可选是否在数据表页左侧列表仅展示有差异的表|`export/result/diff_sheets_{旧版本号}_{新版本号}.txt`|
...
* 导出数据表（exd/root.exl）：读取此游戏文件并格式化导出，此文件记录了全部数据表ID序号，其中ID为负数的为杂项表格（如-1），ID≥0的为普通表格，杂项表格有6000+个，普通表格1000+个。此功能仅输出表格ID序号表，不会真正导出游戏数据表文件，如需导出数据表，请使用「导出」菜单相关功能。
* 找出版本变更的数据表：需要至少导出过两个版本的不同CSV数据表文件，输出txt文档结果会按有变更文件、新版本新增文件、新版本删除文件三项分别列出对应的CSV文件名。若勾选“同时在数据表页面筛选显示有差异的数据表”功能，则对比完成后，程序数据表页面左侧的数据列表会仅显示此次对比中有差异和新增的数据表，需要恢复显示全部数据表，可再次打开此对比功能小窗口，点击“点击还原为显示全部数据表列表”按钮。
* 导出当前数据表内容Diff总结表：以图片/excel表格形式输出当前选择的数据表内容，可选择使用完整表格/个性化表格输出，如果启用了Diff表格对比，则输出结果为Diff结果的表格。输出图片时固定200行数据分割，分多张图片输出。
  ...
  >
  > 额外程序由AI编程，使用python编写并打包为exe单文件程序，需放在`tools/`目录下程序才可调用。
  > 
  > 此功能制作较为简陋，主要用于输出直接截图不方便截取的大量数据行情况。
  > 
  > 如需使用此功能，Release页下载请选择'*.zip'完整包。
  > 
  >> `tools/gen_image_tool.exe`：输出图片时调用。<br>`tools/gen_excel_tool.exe`：输出Excel表格时调用。
...
|**导出全部CSV源文件**|导出当前游戏全部数据表CSV源文件，包括杂项表格|`export/data/{游戏版本号}/*.csv`|
|**导出收藏CSV**|导出当前收藏的数据表CSV文件|`export/data/{游戏版本号}/*.csv`|
|**导出收藏CSV源文件**|导出当前收藏的数据表CSV源文件|`export/data/{游戏版本号}/*.csv`|
...
|**导出全部地图图片**|导出当前游戏全部地图图片文件|`export/map/{区域短编号}-{地名}-{二级地名}.png`|
|**导出全部图片资源**|导出当前游戏全部图片文件|`export/img/ui_{图片ID序号}.png`|

* 导出在后台异步执行，窗口显示进度与结果，导出完成5秒后进度窗口自动关闭。
* CSV源文件：即表格原始的数据内容，导出时不会将引用其他数据表的内容填入其中，日常查看一般不推荐导出为源文件。
* 杂项表：包含任务、过场动画等数据表的文本和配置，一般不需要查看这些表。
...
* `music_list_{游戏版本号}.json`：音乐页面表列表存档。
